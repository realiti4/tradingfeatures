...
class bitmexFunding(bitmexBase):
    ...
    def get(
        ...
        **kwargs
        ...
        df = super(bitmexFunding, self).get(
            query=query, start=start, end=end, interval=interval, *args, **kwargs
        )
    ...
    def get_recent(self, df):  # Recent Funding
        ...
        df_recent_funding = pd.DataFrame(
            [[funding_ts, funding_rate]], columns=["timestamp", "fundingRate"]
        )
        df_recent_funding["timestamp"] = self.to_ts(
            pd.to_datetime(df_recent_funding["timestamp"])
        )
