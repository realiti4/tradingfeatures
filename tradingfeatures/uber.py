...
class Uber:
    ...
    def eval_get(self, limit=1000, **kwargs):
        ...
        with ThreadPoolExecutor(8) as executor:
            for api in self.apis:
                api_columns = (
                    self.column_kwargs[api.name]
                    if api.name in self.column_kwargs
                    else None
                )
                future = executor.submit(
                    api.get, limit=limit, columns=api_columns, **kwargs
                )
    ...
    def get(
        ...
        **kwargs,
        ...
        assert (
            isinstance(datasets[0], list) and len(datasets[0]) == 2
        ), "Use a list of list like [[api_name, api_df], ..]"

        # Remove active hour and fix columns
        for i in range(len(datasets)):
            api_name = datasets[i][0]
            api_columns = (
                self.column_kwargs[api_name] if api_name in self.column_kwargs else None
            )
            if api_columns is None:
                datasets[i][1] = datasets[i][1].loc[: self.current_time() - 1]
            else:
                datasets[i][1] = datasets[i][1][api_columns].loc[
                    : self.current_time() - 1
                ]
        ...
        for i in range(1, len(datasets)):
            ...
            if i == 1:
                df_main = df_main.join(
                    df_join, lsuffix=f"_{name_main}", rsuffix=f"_{name}"
                )
            else:
        ...
        for item in self.columns:
            if item == "volume" or item == "trades":
                df_final[item] = df_main.loc[:, df_main.columns.str.contains(item)].sum(
                    axis=1
                )
            else:
                df_final[item] = df_main.loc[
                    :, df_main.columns.str.contains(item)
                ].mean(axis=1)
